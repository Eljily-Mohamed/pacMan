...
void drawLevel(char** level, int row, int col, RendererParameters *params, Textures *textures)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
        {
            ...
            {
            case 'H':
                drawSpriteOnGrid(textures->textureWall, j, i, 0, params); //mur bleu
                break;
            case '.':
                drawSpriteOnGrid(textures->textureDot, j, i, 0, params);//les points a manger
                break;
            case 'G':
...
SDL_Texture * GetTexture(char* imageName, RendererParameters *params)
{
    ...
    SDL_Texture * texture = SDL_CreateTextureFromSurface(params->renderer, sprite);
     if (!texture) {
        // Handle the error if texture creation fails
        printf("Error creating texture from surface: %s\n", SDL_GetError());
        return NULL;  // Return NULL to indicate texture creation failure
    }
